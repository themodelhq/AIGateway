...
export default function Dashboard() {
  ...
  const PROVIDER_META: Record<string, { icon: string; color: string; description: string; maxTokens: string; cost: string }> = {
    openai:    { icon: "🤖", color: "bg-emerald-50 border-emerald-200 dark:bg-emerald-950/30 dark:border-emerald-800", description: "OpenAI flagship models", maxTokens: "up to 128K", cost: "from $0.15/1M" },
    anthropic: { icon: "🧠", color: "bg-orange-50 border-orange-200 dark:bg-orange-950/30 dark:border-orange-800",   description: "Anthropic Claude models", maxTokens: "up to 200K", cost: "from $0.25/1M" },
                        ...
                        {provModels.map((model) => (
                          <Card key={model.id} className={`p-4 border ${meta.color} hover:shadow-md transition-all duration-200`}>
                            <div className="flex items-start justify-between mb-2">
                              <div>
                                <h4 className="font-medium text-slate-900 dark:text-white text-sm">{model.id}</h4>
                                <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5 capitalize">{model.owned_by}</p>
                              </div>
                              <span className="text-xs bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-300 px-2 py-0.5 rounded-full font-medium">Live</span>
                            </div>
